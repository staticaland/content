...
''' GLOBAL VARS '''
REGISTRATION_ID = demisto.params().get(REGISTRATION_ID_CONST)
# If there's a stored token in integration context, it's newer than current
REFRESH_TOKEN = demisto.getIntegrationContext().get(REFRESH_TOKEN_CONST)
if not REFRESH_TOKEN:
    REFRESH_TOKEN = demisto.params().get(REFRESH_TOKEN_CONST)

ENC_KEY = demisto.params().get(ENCRYPTION_KEY_CONST)

USE_SSL = not demisto.params().get('insecure', True)  # TODO change to false
# TODO change url
...
# where TIME(log_time) Between TIME(TIMESTAMP("2020-02-05 00:31:23")) AND TIME(CURRENT_TIMESTAMP)
...
class Client(BaseClient):
    ...
    def __init__(self):
        ...
        # TODO add proxy
        super().__init__(base_url=TOKEN_RETRIEVAL_URL, headers=headers, verify=USE_SSL)
    ...
    def get_access_token(self):
        ...
        if access_token and stored:
            if int(time.time()) - stored < 60 * 60 - 30:
                return access_token
        oproxy_response = self._http_request("POST",
                                             "/apollo_v2_CDLRefreshTokenFunction",
                                             json_data={'token': get_encrypted(REFRESH_TOKEN, ENC_KEY)})
        ...
        updated_integration_context = {
            ACCESS_TOKEN_CONST: access_token,
            STORED_CONST: int(time.time()),
            API_URL_CONST: api_url,  # TODO use this url
            INSTANCE_ID_CONST: instance_id
        }
        if refresh_token:
    ...
    def query_loggings(self, query_data):
        """
        This function handles all the querying of Cortex Logging service
        """

        query_service = self.initial_query_service()
        query_data.update({'language': 'bigquery'})
        response = query_service.create_query(query_params=query_data)
        query_result = response.json()

        if not response.ok:
            status_code = response.status_code
            message = query_result[-1].get('context')
            raise Exception(f'Error in query to Cortex [{status_code}] - {message}')

        try:
            results = [r.json() for r in query_service.iter_job_results(job_id=query_result.get("jobId"))]
    ...
    def initial_query_service(self):
        api_url = demisto.getIntegrationContext().get(API_URL_CONST,
...
def test_module():
    client = Client()
    test_args = {
        'query': add_instance_id_to_query(client=client,
                                          query=f'SELECT * FROM `firewall.traffic` limit 1')
    }
    client.query_loggings(test_args)
    demisto.results('ok')
...
def query_logs_command():
    ...
    query = add_instance_id_to_query(client, query)
    ...
    response = client.query_loggings(query_data)

    output = []

    for row in response:
        transformed_row = transform_row_keys(row)
...
def add_instance_id_to_query(client: Client, query: str) -> str:
    """
    ...
    integration_context = demisto.getIntegrationContext()
    instance_id = integration_context.get(INSTANCE_ID_CONST)
    query = re.sub(r'(?i)FROM `', f'FROM `{instance_id}.', query)
    return query
...
# TODO documentation
# Where to get costumer name and license_id
# How to query (how does the instance_id added to the queries
# What if table name in the query is not wrapped with ``
